...
def check_user_exists(dbConn, email):
    """
    Checks if a user with the given userid exists in the users table.
    ...
    result = retrieve_one_row(dbConn, sql, [email])
    return result[0] > 0
...
# retrieves and returns the column names for the specified table.
#
def get_column_names(dbConn, table_name):
    """
    Retrieves and returns the column names for the specified table.
    ...
    try:
        ...
        print("datatier.get_column_names() failed:")
        print(str(err))
        raise
